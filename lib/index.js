 ...
 *   - `/api/git-branch/*` Fetch routes for the browser half (inside the
 *     shared Connection channel's authentication fence)
 ...
 * business failures ride the envelope). `rpc.handle` cannot be used here:
 * it resolves `webServer` through the caller fiber's context, which a
 * third-party plugin cannot satisfy in this runtime.
...
export class GitTreeService extends Service {
  ...
  async [Service.init]() {
    // The browser half pulls state through exact Fetch routes under the
    // shared `/api` channel — inside Connection's authentication fence, and
    // the one registration style this runtime resolves from a plugin fiber.
    ...
    this.ctx.inject(['connection'], (connCtx) => {
      for (const endpoint of ['state', 'switch', 'graph']) {
        connCtx.connection.fetch.register({
          path: `/api/git-branch/${endpoint}`,
          methods: ['POST'],
          requestBody: 'buffered',
          fetch: envelopeFetch((payload) => this.handleRpc(endpoint, payload)),
        })
      }
    })
